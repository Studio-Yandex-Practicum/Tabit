...
class DeparmentFactory(AsyncSQLAlchemyFactory):
    ...
    Поля:
        1. name: Обязатьльное поле. Выбирается поочередно из списка available_departments_names.
        2. company_id: Обязательное поле. При создании объекта фабрики: create() create_batch(),
        необходимо передавать в аргументы фабрики.
        3. slug: Обязательное поле. Slug генерируется библиотекой uuid.
