...
- **CI/CD (GitHub Actions)**:
  - [.github/workflows/build_and_push.yaml](../.github/workflows/build_and_push.yaml) - Сборка и публикация Docker-образа:
    - Автоматическая сборка образа при вызове из других workflows
    - Публикация образа в GitHub Container Registry (GHCR)
    - Использование кэширования для ускорения сборки
  - [.github/workflows/pytest.yaml](../.github/workflows/pytest.yaml) - Запуск тестов:
    - Автоматический запуск при push и pull request
    - Установка Poetry и зависимостей
    - Создание .env файла из секретов
    - Запуск тестов через pytest
  - [.github/workflows/ruff.yml](../.github/workflows/ruff.yml) - Проверка кода линтером Ruff:
    - Запуск при создании pull request
    - Проверка стиля кода и форматирование
    - Вывод ошибок в формате GitHub
  - [.github/workflows/stage_deploy.yaml](../.github/workflows/stage_deploy.yaml) - Деплой на Stage-окружение:
    - Автоматический запуск при push в ветку dev
    - Остановка старых контейнеров и очистка окружения
    - Копирование инфраструктурных файлов на сервер
    - Настройка systemd-сервиса
    - Загрузка и запуск Docker-образа
    - Применение миграций базы данных
    - Удаление старых Docker-образов (оставляются последние 5)
...
Инструкции по установке и настройке окружения доступны в [README.md](../README.md#начало-работы).
...
Для работы с проектом необходимо создать файл `.env` в корне проекта на основе [.env.example](../.env.example). Подробнее в разделе [Настройка окружения](../README.md#настройка-окружения).

### Работа с pgAdmin

Для удобного администрирования базы данных используется pgAdmin через Docker. Подробные инструкции в [README.md](../README.md#pgadmin).

### Логирование

Проект использует встроенную систему логирования, настраиваемую через переменные окружения. Подробнее о логировании в [README.md](../README.md#логирование).

### Инфраструктура (Docker)

Инструкции по работе с Docker доступны в [README.md](../README.md#инфраструктура-docker).

### Git Flow и процесс работы

Информация о Git Flow и процессе создания Pull Request доступна в [README.md](../README.md#правила-работы-с-git).
...
- `make fill-license-type` - Создание лицензий для компаний
...
- `make up-pgadmin` - Запуск pgAdmin
- `make down-pgadmin` - Остановка pgAdmin
...
- **Шаблон докстринга**:
  - Шаблоны досктрингов находятся по пути Tabit\docs\code_examples
...
1. **Квест 1: Изучение документации**
  - Изучите [README.md](../README.md)

2. **Квест 2: Настройка окружения**
...
3. **Квест 3: Изучение структуры проекта**
...
5. **Квест 5: Работа с тестовыми данными**
...
6. **Квест 6: Первое изменение**
  ...
  - После слияния отследите выполнение CI/CD pipeline
  - При возникновении ошибок в CI/CD:
    - Проверьте логи тестов
    - Воспроизведите ошибку локально, используя команды для отладки CI/CD
    - Исправьте ошибки и создайте новый PR
  - После успешного прохождения CI/CD убедитесь, что изменения корректно развернуты на stage-окружении
