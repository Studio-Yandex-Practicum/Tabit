...
class TestCompanyValidators:
    ...
    async def test_check_department_name_duplicate_raises(
    ...
    async def test_check_user_company_invalid(self, async_session: AsyncSession, company_for_test):
        """
        Тест проверяет, что при несоответствии slug компании выбрасывается
    ...
    async def test_check_company_and_department_invalid_department(
        ...
        """
        Тест проверяет, что при несуществующем department_id выбрасывается
        HTTPException с кодом 422 и соответствующим сообщением.
    ...
    async def test_validate_company_slug_duplicate(
        ...
        """
        Тест проверяет, что при дублирующемся slug компании выбрасывается
