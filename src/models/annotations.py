...
from src.models.constants import LengthConstants, MiscConstants

int_pk = Annotated[int, mapped_column(primary_key=True, unique=True)]
name_field = Annotated[str, mapped_column(String(LengthConstants.MAX_NAME))]
patronymic_field = Annotated[
    Optional[str], mapped_column(String(LengthConstants.MAX_NAME), nullable=True)
]
license_name_field = Annotated[str, mapped_column(String(LengthConstants.MAX_NAME), unique=True)]
description = Annotated[Optional[str], mapped_column(Text, nullable=True)]
tag_name_field = Annotated[
    str, mapped_column(String(LengthConstants.MAX_SMALL_NAME), unique=True, nullable=False)
]
url_link_field = Annotated[
    Optional[str], mapped_column(String(LengthConstants.FILE_LINK), nullable=True)
]
