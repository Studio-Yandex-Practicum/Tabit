...
from src.utils.constants import DirectoryConstants, MiscConstants, TextErrorConstants
from src.utils.directory import create_folder


async def base64image(str_base64: str, name: str, directory: str = DirectoryConstants.WAIF) -> str:
    """
    ...
    """
    if not (
        isinstance(str_base64, str) and str_base64.startswith(MiscConstants.BASE64_STARTSWITH)
    ):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=TextErrorConstants.BASE64_TYPE,
        )
    try:
        ...
        path_directory = f'{DirectoryConstants.MEDIA}/{directory}'
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=TextErrorConstants.BASE64_FATAL.format(
                image=directory,
